...
- **Break not firing?** Ensure you clicked **"inspect"** in `chrome://inspect` _before_ the script reached the `evaluate` call.
...
## 5. Debugging inside `evaluate` (Bridge Pattern)
...
```javascript
await evaluate(() => {
  if (window.TV_CONFIG.isDebug) debugger; // Only pauses if TV_DEBUG=1
...
The `evaluate` function automatically appends `//# sourceURL=FunctionName.js` based on the passed function's name.

```javascript
export async function myTask() {
  return evaluate(function myTask() {
    if (window.TV_CONFIG.isDebug) debugger;
    // ...
  });
}
```
*Note: The script will appear as `myTask.js` in the DevTools **Sources** tab.*
