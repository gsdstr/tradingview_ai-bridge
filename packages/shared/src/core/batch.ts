...
async function waitForChartReady(symbol?: string | null): Promise<boolean> {
  // Simple check for symbol match or just wait
...
export interface BatchRunOptions {
  symbols: string[];
  timeframes?: string[];
  action: "screenshot" | "get_ohlcv" | "get_strategy_results";
  delay_ms?: number;
  ohlcv_count?: number;
}
...
export async function batchRun(options: BatchRunOptions): Promise<BatchRunResult> {
  ...
  const results: BatchIterationResult[] = [];
