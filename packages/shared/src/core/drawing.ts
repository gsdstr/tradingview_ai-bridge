...
export async function drawShape(options: DrawShapeOptions): Promise<any> {
  const { shape, point, point2, overrides: overridesRaw, text } = options;
...
export async function listDrawings(): Promise<any> {
  const apiPath = await getChartApi();
...
export async function getProperties(options: { entity_id: string }): Promise<any> {
...
export async function removeOne(options: { entity_id: string }): Promise<any> {
  const { entity_id } = options;
  const apiPath = await getChartApi();
  const result = await evaluate(`
    (function() {
...
export async function clearAll(): Promise<any> {
  const apiPath = await getChartApi();
