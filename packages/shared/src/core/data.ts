...
export async function getOhlcv(options: GetOhlcvOptions = {}): Promise<any> {
  const { count, summary } = options;
...
export async function getStrategyResults(): Promise<any> {
...
export async function getDepth(): Promise<any> {
  const data = await evaluate(`
    (function() {
...
export async function getStudyValues(): Promise<any> {
  const data = await evaluate(`
    (function() {
      var chart = window.TradingViewApi._activeChartWidgetWV.value()._chartWidget;
      var model = chart.model();
